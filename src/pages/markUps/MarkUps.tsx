...
import { IMarkUpsList, MarkUpsService } from './MarkUpsService';

export const MarkUps: React.FC = () => {
  const [searchParams, setSearchParams] = useSearchParams();
  const { debounce } = useDebounce();
  const navigate = useNavigate();

  const [rows, setRows] = useState<IMarkUpsList[]>([]);
  ...
  useEffect(() => {
    ...
    debounce(() => {
      MarkUpsService.getAll(pagina, busca).then(res => {
        setIsLoading(false);
        console.log(res);
    ...
    <LayoutBase
      ...
      toolBar={
        <ToolBarSearch
          ...
          searchText={busca}
          textNewButton="Novo"
          clickingNewButton={() => navigate('/markUps/detalhe/novo')}
          ...
          <TableBody>
            {rows.map(row => (
              <TableRow key={row.id}>
                ...
                <TableCell align="center">{row.marketing}%</TableCell>
                <TableCell align="center">{row.promoters}%</TableCell>
